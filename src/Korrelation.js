...
const Korrelation = ({
  ...
  useEffect(() => {
    if (
      ...
      fetch(`/assets/np_${selectedSubject}_reg.json`)
        .then((response) => response.json())
        .then((data) => {
          const referenceDataRaw = data.map(
            (d) => d[`${selectedMetric}_np_${selectedSubject}_${actualYear}`]
          );

          // console.log("Reference data:", referenceDataRaw);

          const coefficients = variables.map((varName) => {
            const variableDataRaw = data.map(
              (d) => d[`${varName}_${actualYear}`]
            );
            ...
            const filteredData = referenceDataRaw.reduce((acc, val, index) => {
              if (
                val !== null &&
                !isNaN(val) &&
                variableDataRaw[index] !== null &&
                !isNaN(variableDataRaw[index])
              ) {
  ...
  const chartOptions = {
    ...
    title: {
      text: `Korrelationskoefficient mellan ${selectedMetricName.toLowerCase()} och utvalda variabler, ${
        2000 + actualYear
      }`,
      align: "left",
    },
  ...
  return (
    <div className="chart-container">
      <div className="description-container">
        <h2>
          Korrelation mellan {selectedMetricName.toLowerCase()} och utvalda
          variabler
        </h2>
        <p>
          Diagrammet visar korrelationskoefficienten mellan{" "}
          {selectedMetricName.toLowerCase()} och utvalda variabler för det valda
          året. Använd skjutreglaget under diagrammet för att byta år.
        </p>
      </div>
